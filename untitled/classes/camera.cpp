...
void Camera::set_camera_direction(glm::vec3 look_at_position)
{
    ...
    m_camera_yaw = std::atan2(m_camera_forward.x, m_camera_forward.z);
    m_camera_pitch = std::atan2(m_camera_forward.y, sqrt(m_camera_forward.x*m_camera_forward.x + m_camera_forward.z*m_camera_forward.z));
}
...
glm::mat4 Camera::update(QSet<int> *keys)
{
    ...
    // EARLY RETURN if no movement/rotation keys are pressed
    if (!update_needed(keys)) {
    ...
    m_last_time = m_current_time; // instead of calling m_timer.elapsed() again
    return create_view_matrix();
}
...
bool Camera::update_needed(QSet<int> *keys)
{
    static const QList<int> cameraKeys = {
